...
def test_x_project_context_overrides_project_filter() -> None:
    """Engrams stored with owner_type='project'/owner_id='proj-v1'
    must appear when X-Project-Context: proj-v1 is sent.

    We store via the HTTP /store endpoint so the embedding is generated with
    the same encoder as the search query — ensuring high cosine similarity.
    The admin token resolves to owner_type=None/is_admin=True; we patch
    owner_type to 'project' directly in the DB after storing.
    """
    # Store via the HTTP route — real embedding generated by the live encoder
    store_resp = _client.post(
        "/store",
        json={
            "context_id": "proj-engram-v1-001",
            "agent": "test",
            "text": "unique project context engram xyzzy",
            "epistemic_truths": [],
            "core_concepts": [],
    ...
    # Patch the stored engram to owner_type='project', owner_id='proj-v1'
    # so the X-Project-Context search path picks it up.
    with _db._conn() as conn:
        conn.execute(
            "UPDATE mirror_engrams SET owner_type=?, owner_id=? WHERE context_id=?",
            ("project", "proj-v1", "proj-engram-v1-001"),
        )
