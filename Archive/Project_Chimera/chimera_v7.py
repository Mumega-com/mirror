...
# ==============================================================================
# PROJECT CHIMERA V7 - SPARSE RESERVOIR (ReLU)
# ==============================================================================

CONFIG = {
    'batch_size': 64,
    'latent_dim': 1024,           # Wider for sparsity
    'spectral_radius': 1.2,
    'input_scale': 0.8,
    'dt': 0.1,                    
    'integration_steps': 15,
    'sparsity_threshold': 0.0,    # Implicit via ReLU
    
    'lr_readout': 0.002,
    'lr_reservoir': 0.0,          # FROZEN
...
# ==============================================================================
# FRC 840.003 - CHIMERA RESERVOIR (SPARSE)
# ==============================================================================

class SparseChimeraBlock(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        self.relu = nn.ReLU()
        
        # Reservoir Weights - Sparse Initialization
        W_res = torch.randn(dim, dim)
        
        # Pruning for structural sparsity
        mask = (torch.rand(dim, dim) > 0.9).float() # 90% sparse
        W_res = W_res * mask
        
        # Spectral radius normalization
        try:
            # Approximate spectral radius for sparse matrix
            u, s, v = torch.svd(W_res) # Dense SVD is heavy but fine for 1024
            s[0] = CONFIG['spectral_radius']
            W_res = torch.mm(torch.mm(u, torch.diag(s)), v.t())
        except:
             # Fallback simple scaling
             W_res = W_res * (CONFIG['spectral_radius'] / torch.norm(W_res))
        ...
        self.bias = nn.Parameter(torch.randn(dim) * -0.5, requires_grad=False) # Negative bias helps sparsity

    def forward(self, h, u_in):
        # Euler with ReLU
        drive = torch.mm(h, self.W_res) + torch.mm(u_in, self.W_in) + self.bias
        dh = -h + self.relu(drive)
...
class ChimeraNet(nn.Module):
    def __init__(self):
        ...
        # 2. FROZEN RESERVOIR
        self.reservoir = SparseChimeraBlock(self.latent_dim)
...
def train_chimera_v7(tasks):
    print("\nInitializing Project Chimera V7 (Sparse/ReLU)...")
    ...
    for task_id, task in enumerate(tasks):
        ...
        print(f"\n>>> Task {task_id} (V7 Sparse - Active: {active_digits}) <<<")
...
if __name__ == "__main__":
    ...
    acc_matrix = train_chimera_v7(tasks)
    ...
    print("\n" + "="*60)
    print("PROJECT CHIMERA V7: FINAL REPORT")
    print("="*60)
