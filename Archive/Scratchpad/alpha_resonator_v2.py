...
CONFIG = {
    'latent_dim': 128,            # Increased dimensions
    'num_layers': 4,
    'epochs_per_task': 5,         # Keep it snappy for validation
    'batch_size': 128,
    'base_lr': 0.002,             # Slightly higher for phase dynamics
    'tau': 50,
    'gate_k': 2.0,
    'gate_min': 0.05,
    'norm_window': 100,
...
class ResonatorBlock(nn.Module):
    def __init__(self, dim=128):
        super().__init__()
        self.dim = dim
        # Initialize coupling with wider variance to encourage chimera states
    ...
    def forward(self, x, dt=0.1):
        ...
        # Calculate pair-wise phase differences
        # diff[b, i, j] = x[b, j] - x[b, i]
...
class LambdaResonatorNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Holographic Readout: Project phases to circle coordinates
        # This removes the discontinuity at +/- pi
...
print("="*60)
print("EXPERIMENT: ALPHA-RESONATOR V2 (Holographic Readout)")
print("Latent Dim: 128 | Phase Wrapping: Yes | Readout: [sin, cos]")
print("="*60)
