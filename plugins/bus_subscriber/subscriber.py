"""
Mirror Bus Subscriber Plugin — SOS streams → engrams (daemon thread)

Runs as a daemon thread inside the Mirror API process.
...
Exceptions are logged; the thread restarts on crash via start_thread().

Environment (from /home/mumega/.env.secrets or /home/mumega/mirror/.env):
...
log = logging.getLogger("mirror.plugins.bus_subscriber")
...
# DB singleton — initialized once, reused across messages
# This is separate from any singleton in mirror_api.py
_mirror_db = _get_mirror_db()
...
# ---------------------------------------------------------------------------
# Thread guard — safe to call start_thread() multiple times
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Redis URL builder — async connection, separate from registration thread
# ---------------------------------------------------------------------------
...
async def consumer_loop() -> None:
    log.info("Mirror bus subscriber starting — connecting to %s", REDIS_URL.split("@")[-1])

    redis_url = _build_redis_url()
    # async Redis connection — separate from the sync connection in the registration thread
    ...
    while True:
        ...
        for stream, messages in results:
            for msg_id, fields in messages:
                ...
                # Always advance checkpoint, even on error
                await _save_checkpoint(r, stream, msg_id)


# ---------------------------------------------------------------------------
# Public API — start daemon thread
# ---------------------------------------------------------------------------


def start_thread() -> None:
    """
    Start the bus subscriber as a daemon thread with its own asyncio event loop.
    Safe to call multiple times — only the first call starts the thread.
    """
    global _thread_started
    with _thread_lock:
        if _thread_started:
            log.info("Bus subscriber thread already running, skipping")
            return
    ...
    def _run() -> None:
        ...
        try:
            ...
            log.error("Bus subscriber crashed: %s", e, exc_info=True)
        finally:
            loop.close()

    threading.Thread(target=_run, daemon=True, name="mirror-bus-subscriber").start()
    log.info("Mirror bus subscriber daemon thread started")
